...
var searchFn = function () {
    ...
    var render = function (results) {
        ...
        for (var i = 0; i < results.length && i < limit; i += 1) {
            ...
            var resultPane = openAnchor +
                "<div class=\"items\">" +
                "<div class=\"h3\">" +
                result.showTitle + "</div>" +
                ("<p class=\"\">" + result.showDescription + 
                ("<time class=\"\"><span> Last Update : </span>" + result.showDate + "</time>") +
                "</p>") +
                "</div></a>";
            $("#search-results").append(resultPane);
        }
    };
    ...
    var search = function (terms) {
        ...
        if (results.length) {
            ...
            $("#search-total").html("<p>" + resultsMessage + "</p>");
            render(results);
        }
        else {
            $("#search-total").html('<p>No items found.</p>');
        }
    };
