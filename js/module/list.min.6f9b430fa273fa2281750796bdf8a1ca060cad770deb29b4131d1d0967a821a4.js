...
function allcardDropdown(){for(let x=0;x<cl('action-drop').length;x++){cl('action-drop')[x].addEventListener('click',event=>{let target=cl('action-drop')[x].getAttribute('data-dropdown');id(target+"-dropdown").classList.toggle('hide');cl('action-drop')[x].classList.toggle('close');})}}
if(screen.width<960){filterBox();filterCat();allcardDropdown();}